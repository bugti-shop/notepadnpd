...
import {
  setGlobalPatternLock,
  hasGlobalPatternLock,
  removeGlobalPatternLock,
...
interface GlobalPatternSetupSheetProps {
  isOpen: boolean;
  onClose: () => void;
  onPatternSet?: () => void;
}

export const GlobalPatternSetupSheet = ({
  isOpen,
  onClose,
  onPatternSet,
}: GlobalPatternSetupSheetProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      // Check for existing global pattern
      hasGlobalPatternLock().then(setHasExistingPattern);
      // Reset state
      setStep('draw');
      setPattern([]);
      setConfirmPattern([]);
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      await setGlobalPatternLock(pattern, securityQuestion, securityAnswer.trim());
      toast.success('Global pattern lock enabled!');
      onPatternSet?.();
      onClose();
    } catch (error) {
      console.error('Error setting global pattern lock:', error);
  ...
  const handleRemovePattern = async () => {
    ...
    setIsLoading(true);
    try {
      await removeGlobalPatternLock();
      toast.success('Global pattern lock removed');
      onPatternSet?.();
      onClose();
    } catch (error) {
      console.error('Error removing global pattern lock:', error);
  ...
  const getStepTitle = () => {
    switch (step) {
      case 'draw':
        return hasExistingPattern ? 'Change App Pattern' : 'Set App Pattern';
      case 'confirm':
  ...
  const getStepDescription = () => {
    switch (step) {
      case 'draw':
        return 'This pattern will protect ALL notes in the app';
      case 'confirm':
          ...
          {step === 'security' && (
            <div className="space-y-4 animate-fade-in">
              ...
              >
                ...
                {isLoading ? 'Saving...' : 'Enable App-Wide Pattern Lock'}
              </Button>
            </div>
          )}
