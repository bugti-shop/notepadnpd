...
import {
  verifyPatternLock,
  getPatternSecurityQuestion,
  verifyPatternSecurityAnswer,
  resetPatternLock,
...
interface PatternUnlockSheetProps {
  isOpen: boolean;
  onClose: () => void;
  noteId: string;
  onUnlocked: () => void;
...
export const PatternUnlockSheet = ({
  isOpen,
  onClose,
  noteId,
  onUnlocked,
}: PatternUnlockSheetProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      getPatternSecurityQuestion(noteId).then(setSecurityQuestion);
    }
  }, [isOpen, noteId]);
  ...
  const handlePatternAttempt = async (pattern: number[]) => {
    ...
    try {
      const isValid = await verifyPatternLock(noteId, pattern);
  ...
  const handleVerifySecurityAnswer = async () => {
    ...
    setIsVerifying(true);
    try {
      const isValid = await verifyPatternSecurityAnswer(noteId, securityAnswer);
      ...
      <SheetContent side="bottom" className="rounded-t-3xl max-h-[90vh] overflow-y-auto">
        <SheetHeader className="mb-4">
          ...
          <p className="text-sm text-muted-foreground mt-2 pl-10">
            {step === 'pattern' && 'Draw your pattern to unlock this note'}
              ...
              {securityQuestion ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">
                    No security question was set for this note's pattern lock.
                  </p>
